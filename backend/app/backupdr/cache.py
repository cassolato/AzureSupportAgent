"""Persistent server-side cache for Backup & DR Coverage snapshots.

Coverage gathers per-resource backup/DR facts (slow), so snapshots are cached on the Azure
Files volume (``backend/.data/backupdr_coverage_cache.json``), keyed by ``(tenant,
scope_kind, scope_id)``, with a per-key lock to coalesce concurrent recomputes."""
...
_PATH = Path(__file__).resolve().parents[2] / ".data" / "backupdr_coverage_cache.json"
