...
def _ticket_body(chat: Chat, messages: list[Message]) -> str:
    """Render the WHOLE conversation (start to finish) into a rich Markdown ticket body."""
    title = (chat.title or "Azure Support Agent conversation").strip()
    lines: list[str] = [
        f"# {title}",
        "",
        f"*Exported from Azure Support Agent on {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}.*",
        "",
        "---",
        "",
    ]
    ...
    for m in messages:
        if m.role not in ("user", "assistant"):
            continue
        who = role_label.get(m.role, m.role.title())
        ts = m.created_at.strftime("%Y-%m-%d %H:%M UTC") if getattr(m, "created_at", None) else ""
        meta = f" · {m.model}" if (m.role == "assistant" and getattr(m, "model", "")) else ""
        lines.append(f"### {who}{(' · ' + ts) if ts else ''}{meta}")
        lines.append("")
        lines.append((m.content or "").strip() or "_(no text)_")
        lines.append("")
    lines.append("---")
    lines.append("")
    lines.append("_Generated by Azure Support Agent._")
    return "\n".join(lines)
...
@router.get("/ticket-connectors")
async def ticket_connectors(principal: Principal = Depends(get_principal)):
    """Enabled ticketing connectors (ServiceNow / Jira / XSOAR) the user can send a chat to.
    Non-secret metadata only."""
    from app.connectors import tickets

    return {"connectors": tickets.ticket_connectors()}


@router.post("/{chat_id}/send-ticket")
async def send_ticket(
    chat_id: str,
    payload: SendTicketRequest,
    principal: Principal = Depends(get_principal),
    db: AsyncSession = Depends(get_db),
):
    """Create a ticket in the chosen connector with the ENTIRE conversation as a rich body, and
    attach a PDF transcript of the whole chat. Returns the created ticket number + deep link."""
    from app.connectors import chat_pdf, tickets
    ...
    if not rows:
        raise HTTPException(status_code=400, detail="This chat has no messages to send.")

    title = (chat.title or "").strip()
    if not title:
        first_user = next((m for m in rows if m.role == "user"), None)
        title = (first_user.content if first_user else "Azure Support Agent conversation").strip()
    title = title.splitlines()[0][:160] if title else "Azure Support Agent conversation"
    body = _ticket_body(chat, list(rows))

    # Render the whole conversation to a PDF transcript to attach alongside the body. PDF
    # generation is best-effort: a failure must not block ticket creation.
    pdf_bytes: bytes | None = None
    try:
        msgs = [{"role": m.role, "content": m.content, "created_at": m.created_at,
                 "model": getattr(m, "model", "")} for m in rows]
        pdf_bytes = await asyncio.to_thread(chat_pdf.build_chat_pdf, (chat.title or title), msgs)
    except Exception as exc:  # noqa: BLE001
        logger.warning("Chat PDF generation failed for %s: %s", chat_id, format_error(exc))

    safe = re.sub(r"[^A-Za-z0-9._-]+", "-", (chat.title or "conversation")).strip("-")[:60] or "conversation"
    result = await tickets.create_ticket(
        payload.connector_id, title, body, pdf_bytes=pdf_bytes, pdf_name=f"{safe}.pdf",
    )
    ...
    db.add(
        AuditLog(
            ...
            action="chat.send_ticket",
            target=chat_id,
            metadata_json={
                "connector_id": payload.connector_id,
                "ok": bool(result.get("ok")),
                "ticket": result.get("number", ""),
    ...
    if not result.get("ok"):
        raise HTTPException(status_code=502, detail=result.get("error", "Ticket creation failed."))
    return result
